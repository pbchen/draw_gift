/* [ ---- Gebo Admin Panel - floating list header ---- ] */
	...
	gebo_floating_header = {
		basic: function() {
            ...
            $list_a_outer.append('<div class="slide-up slide-nav"><i class="glyphicon glyphicon-chevron-up"></i></div><div class="slide-down slide-nav"><i class="glyphicon glyphicon-chevron-down"></i></div>');
		...
		scrollto: function() {
			...
			$list_b.find('dt').each(function(i){
				...
				$('#list-buttons').append('<button data-header="'+i+'" class="btn btn-xs btn-default bstr_ttip" title="'+dd_length+'">'+$(this).text()+'</button>');
			});
			if(!is_touch_device()){
				//* add tooltip 
			    $('.bstr_ttip').tooltip();
