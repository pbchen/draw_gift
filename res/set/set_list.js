...
$(document).ready(function () {
    ...
    //编辑提示框
    $('#edit-set-modal').modal({
        backdrop: 'static',
        show: false
    });

    var ajax_source = "/set/set_list_page";
    //列表datatable
    var oTable = $('#set_tb').dataTable({
    ...
    function downUpOper(clickId) {
        ...
        if (flag) {
            $.post('/set/update_set?', {ids: ids, status: status}, function (ret) {
                var d = $.parseJSON(ret);
                if (d.errCode == 0) {
    ...
    //上架
    $("#stop-set").click(function () {
        downUpOper('stop-set');
    });
    //下架
    $("#start-set").click(function () {
        downUpOper('start-set');
    });
    
    //新建
    $("#add-set").click(function(){
        $("#add-set-modal").modal('show');
        $("#add-set-bnt").die().live('click',function(){
           ...
           if(flag){
               $.post('/set/add_set?',{name:name,remark:remark},function(ret){
                   var d = $.parseJSON(ret);
                   $("#add-set-modal").modal('hide');
                   if (d.errCode==0) {
                        alertSuccess("#alert-success",'');
                        var oSettings = oTable.fnSettings();
                        oSettings.sAjaxSource = ajax_source + getSearchParams();
    ...
    $("a.edit").die().live('click',function(){
        ...
        $("#edit-set-bnt").die().live('click',function(){
            $(".alert-label-error").text('');
            var flag = true;
