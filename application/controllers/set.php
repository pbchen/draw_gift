...
class set extends CI_Controller {
    ...
    public function add_set(){
        ...
        if($id=$this->set_model->add_set($d)){
    ...
    public function edit_set(){
        ...
        if($check_info=$this->set_model->check_set_update(array($id),$d['status'])){
            json_out_put(return_model('2022', $check_info, NULL));
        }
        $affect_row = $this->set_model->update_set_info($d,array('id'=>$id));
    ...
    public function update_set(){
        ...
        $d['status'] = $this->input->post('status');
        if($check_info=$this->set_model->check_set_update($ids,$d['status'])){
            json_out_put(return_model('2022', $check_info, NULL));
        }
        $this->db->where_in('id',$ids);
        $aff_row = $this->set_model->update_set_info($d);
    ...
    public function set_list_page(){
        $d = $this->set_model->set_page_data($this->data_table_parser);
